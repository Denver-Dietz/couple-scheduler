    ...
    <div className="flex flex-col gap-6">
      ...
      <div>
        <div className="flex items-center justify-between" style={{ marginBottom: '0.75rem' }}>
          ...
          {isDashboardOwner && (
            <button className="btn btn-outline" aria-label={showCommitForm ? "Close Appointment form" : "Add Appointment"} onClick={() => setShowCommitForm(!showCommitForm)}
            ...
            {c_list.map(c => (
              <div key={c.id} className="card flex items-center justify-between"
                ...
                {c.user_id === currentUser || c.user_id === 'both' ? (
                  <button aria-label={`Delete appointment: ${c.title}`} onClick={() => { api.deleteCommitment(c.id); refresh(); }}
      ...
      <div>
        <div className="flex items-center justify-between" style={{ marginBottom: '0.75rem' }}>
          ...
          {isDashboardOwner && (
            <button className="btn btn-outline" aria-label={showGoalForm ? "Close Goal form" : "Add Goal"} onClick={() => setShowGoalForm(!showGoalForm)}
              style={{ padding: '0.3rem 0.6rem', fontSize: '0.8rem' }}>
            ...
            {g_list.map(g => (
              <div key={g.id} className="card flex items-center justify-between"
                ...
                {g.user_id === currentUser || g.user_id === 'both' ? (
                  <button aria-label={`Delete goal: ${g.title}`} onClick={() => { api.deleteGoal(g.id); refresh(); }}
                    style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: '4px' }}>
                    <Trash2 size={14} />
                  </button>
                ) : (
      ...
      <div>
        <div className="flex items-center justify-between" style={{ marginBottom: '0.75rem' }}>
          ...
          {isDashboardOwner && (
            <button className="btn btn-outline" aria-label={showProjectForm ? "Close Project form" : "Add Project"} onClick={() => setShowProjectForm(!showProjectForm)}
              style={{ padding: '0.3rem 0.6rem', fontSize: '0.8rem' }}>
            ...
            {p_list.map(p => (
              <div key={p.id} className="card flex items-center justify-between"
                ...
                {p.user_id === currentUser || p.user_id === 'both' ? (
                  <button aria-label={`Delete project: ${p.title}`} onClick={() => { api.deleteProject(p.id); refresh(); }}
                    style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: '4px' }}>
                    <Trash2 size={14} />
                  </button>
                ) : (
