        ...
        <div className="flex items-center justify-between" style={{ marginBottom: '0.75rem' }}>
          ...
          {isDashboardOwner && (
            <button className="btn btn-outline" onClick={() => setShowCommitForm(!showCommitForm)}
              aria-label={showCommitForm ? "Close appointment form" : "Add appointment"} title={showCommitForm ? "Close appointment form" : "Add appointment"}
              ...
              <div key={c.id} className="card flex items-center justify-between"
                ...
                {c.user_id === currentUser || c.user_id === 'both' ? (
                  <button onClick={() => { api.deleteCommitment(c.id); refresh(); }}
                    aria-label={`Delete appointment ${c.title}`} title="Delete appointment"
        ...
        <div className="flex items-center justify-between" style={{ marginBottom: '0.75rem' }}>
          ...
          {isDashboardOwner && (
            <button className="btn btn-outline" onClick={() => setShowGoalForm(!showGoalForm)}
              aria-label={showGoalForm ? "Close goal form" : "Add goal"} title={showGoalForm ? "Close goal form" : "Add goal"}
              style={{ padding: '0.3rem 0.6rem', fontSize: '0.8rem' }}>
              ...
              <div key={g.id} className="card flex items-center justify-between"
                ...
                {g.user_id === currentUser || g.user_id === 'both' ? (
                  <button onClick={() => { api.deleteGoal(g.id); refresh(); }}
                    aria-label={`Delete goal ${g.title}`} title="Delete goal"
                    style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: '4px' }}>
                    <Trash2 size={14} />
                  </button>
                ) : (
        ...
        <div className="flex items-center justify-between" style={{ marginBottom: '0.75rem' }}>
          ...
          {isDashboardOwner && (
            <button className="btn btn-outline" onClick={() => setShowProjectForm(!showProjectForm)}
              aria-label={showProjectForm ? "Close project form" : "Add project"} title={showProjectForm ? "Close project form" : "Add project"}
              style={{ padding: '0.3rem 0.6rem', fontSize: '0.8rem' }}>
              ...
              <div key={p.id} className="card flex items-center justify-between"
                ...
                {p.user_id === currentUser || p.user_id === 'both' ? (
                  <button onClick={() => { api.deleteProject(p.id); refresh(); }}
                    aria-label={`Delete project ${p.title}`} title="Delete project"
                    style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: '4px' }}>
                    <Trash2 size={14} />
                  </button>
                ) : (
