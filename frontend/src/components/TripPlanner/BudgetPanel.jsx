      ...
      <div className="flex flex-col gap-2 flex-1 overflow-y-auto pr-2 mt-2">
        {budget.map(item => (
          <div key={item.id} className="p-3 flex items-center justify-between" style={{ background: 'var(--bg-panel)', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
            ...
            <button aria-label="Delete budget item" onClick={() => handleDelete(item.id)} className="btn btn-outline" style={{ padding: '4px', border: 'none' }}><Trash2 size={14}/></button>
          </div>
        ))}
      </div>

      <div className="flex gap-2 mt-auto" style={{ borderTop: '1px solid var(--border-color)', paddingTop: '1rem' }}>
        <input aria-label="Budget category" className="input" placeholder="Category" value={category} onChange={e => setCategory(e.target.value)} style={{ flex: 1 }} />
        <input aria-label="Budget estimated cost" className="input" type="number" placeholder="Est $" value={estimated} onChange={e => setEstimated(e.target.value)} style={{ width: '80px' }} />
        <button aria-label="Add budget item" className="btn btn-primary" onClick={handleAdd}><Plus size={16}/></button>
      </div>
    </div>
  );
}
