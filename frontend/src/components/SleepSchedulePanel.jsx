          ...
          return (
            <div key={s.id} style={{ 
              ...
              {s.user_id === currentUser && (
                <button onClick={() => { api.deleteSleepSchedule(s.id); refresh(); }} aria-label="Delete sleep schedule" className="btn btn-outline" style={{ padding: '0.4rem', border: 'none' }}>
        ...
        showForm ? (
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: '8px', padding: '1rem' }}>
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setShowForm(false)} aria-label="Close sleep schedule form" style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)' }}><X size={16}/></button>
            </div>
