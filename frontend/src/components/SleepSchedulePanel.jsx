          ...
          return (
            <div key={s.id} style={{ 
              ...
              {s.user_id === currentUser && (
                <button aria-label="Delete schedule" onClick={() => { api.deleteSleepSchedule(s.id); refresh(); }} className="btn btn-outline" style={{ padding: '0.4rem', border: 'none' }}>
        ...
        showForm ? (
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: '8px', padding: '1rem' }}>
            <div className="flex justify-between items-center mb-4">
              ...
              <button aria-label="Close form" onClick={() => setShowForm(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)' }}><X size={16}/></button>
            </div>
