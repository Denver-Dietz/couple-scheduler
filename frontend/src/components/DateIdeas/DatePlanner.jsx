...
const SuccessModal = ({ onClose }) => (
  <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1100 }}>
    <div className="card glass-panel" style={{ padding: '2rem', textAlign: 'center', maxWidth: '300px' }}>
      ...
      <button className="btn" onClick={onClose} style={{ marginTop: '1rem', width: '100%' }}>OK</button>
    </div>
  </div>
);
...
const DatePlannerForm = ({ formData, handleChange, handleSavePlan }) => (
  <>
...
export default function DatePlanner({ onBack, scheduledDates, onSchedule, datesData }) {
  ...
  if (showSuccess) {
    return <SuccessModal onClose={() => { setShowSuccess(false); onSchedule(); }} />;
  }
