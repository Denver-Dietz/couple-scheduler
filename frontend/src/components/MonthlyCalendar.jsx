  ...
  return (
    <div className="monthly-calendar-container">
      ...
      <div className="calendar-header glass-panel">
        <button aria-label="Previous month" className="btn-icon" onClick={prevMonth}><ChevronLeft /></button>
        <h3 className="calendar-title flex items-center gap-2">
          <CalendarIcon className="text-accent" />
          {monthName} {year}
        </h3>
        <button aria-label="Next month" className="btn-icon" onClick={nextMonth}><ChevronRight /></button>
      </div>
      ...
      {selectedEvent && editData && (
        <div className="modal-overlay">
          <div className="modal-content card">
            ...
            <button aria-label="Close" className="close-btn" style={{ position: 'absolute', top: '1rem', right: '1rem', background: 'transparent', border: 'none', color: 'var(--text-muted)', cursor: 'pointer' }} onClick={() => setSelectedEvent(null)}>
              <X size={20} />
            </button>
          </div>
        </div>
