                    ...
                    return (
                      <div key={s.id} style={{ 
                        ...
                        {s.user_id === currentUser && (
                          <button aria-label={`Delete work shift`} onClick={async () => { await api.deleteWorkShift(s.id); refresh(); }} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, color: 'inherit' }}>
                            <Trash2 size={10} />
                          </button>
                        )}
                      </div>
