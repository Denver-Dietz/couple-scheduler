...
def update_trip(trip_id: str, payload: TripUpdate):
    with get_db() as conn:
        ...
        updates = []
        params = []
        allowed_keys = {"name", "dates", "destination", "cover_photo", "mood_tags", "progress"}
...
def update_itinerary_item(item_id: str, payload: TripItineraryUpdate):
    with get_db() as conn:
        ...
        updates = []
        params = []
        allowed_keys = {"day", "title", "time", "location", "notes"}
        for k, v in payload.model_dump(exclude_unset=True).items():
            if k in allowed_keys:
                updates.append(f"{k} = ?")
                params.append(v)
...
def update_budget_item(item_id: str, payload: TripBudgetUpdate):
    with get_db() as conn:
        ...
        updates = []
        params = []
        allowed_keys = {"category", "estimated", "actual", "paid_by"}
        for k, v in payload.model_dump(exclude_unset=True).items():
            if k in allowed_keys:
                updates.append(f"{k} = ?")
                params.append(v)
